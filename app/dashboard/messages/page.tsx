...
export default function MessagesPage() {
  ...
  // Filter conversations based on orders of logged in user
  useEffect(() => {
    if (!user) return;
    const userOrders = orders.filter(
      (order) =>
        order.client?.id === user.uid || order.gig?.freelancer?.id === user.uid
    );
    const allowedUserIds = new Set<string>();
    userOrders.forEach((order) => {
      if (order.client?.id) allowedUserIds.add(order.client.id);
      if (order.gig?.freelancer?.id) allowedUserIds.add(order.gig.freelancer.id);
    });
  ...
  // Join socket rooms and handle incoming messages
  useEffect(() => {
    if (!socket) return;
                ...
                <div className="p-4 border-b flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h2 className="text-xl font-bold">
                      {selectedConversation.participants.find((p) => p.id !== user.uid)?.name ?? 'Unknown'}
                    </h2>
                    ...
                    {selectedConversation.messages.map((message) => (
                      ...
                      >
                        ...
                        >
                          ...
                          {message.senderId !== user.uid && <MessageTranslation text={message.text} />}
                        </div>
                      </div>
                    ))}
                  </div>
