...
export default function FreelancerProfilePage({ params }: { params: { freelancerId: string } }) {
  ...
  const freelancer: User | undefined = users.find(
    (u) => u._id === freelancerId
  );

  if (!freelancer || freelancer.role !== "seller") {
    notFound();
  }

  const freelancerGigs = allGigs.filter(
    (gig) => gig.userId === freelancer._id
  );

  const allReviews: Review[] = freelancerGigs.flatMap((gig) => gig.reviews || []);

  const totalReviews = freelancerGigs.reduce(
  ...
  const averageRating =
    totalReviews > 0
      ? freelancerGigs.reduce(
          (acc, gig) => acc + ((gig.rating ?? 0) * (gig.totalReviews ?? 0)),
          0
        ) / totalReviews
      : 0;
    ...
    <div className="container mx-auto py-12 px-4 md:px-6">
      <div className="grid md:grid-cols-4 gap-8 lg:gap-12">
            ...
            <CardContent className="space-y-3 text-sm pt-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <span>
                  Member since{" "}
                  <span className="font-medium">
                    {formatDate(freelancer.memberSince)}
                  </span>
                </span>
              </div>
            </CardContent>
        ...
        <div className="md:col-span-3">
          ...
          <div>
            ...
            {freelancerGigs.length > 0 ? (
              ...
              <p className="text-muted-foreground">
                This freelancer doesn't have any active gigs yet.
              </p>
            )}
          </div>
          ...
          <div>
            <h2 className="text-3xl font-bold mb-6">
              What People Are Saying ({allReviews.length})
            </h2>
            {allReviews.length > 0 ? (
              ...
              <p className="text-muted-foreground">
                No reviews for this freelancer yet.
              </p>
            )}
          </div>
        </div>
